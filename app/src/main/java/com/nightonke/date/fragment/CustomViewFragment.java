                ...
                if (isFrom) {
                    ...
                    if (to.before(from)) {
                        ...
                        fromDate.setText(" ● " +
                                ...
                                StatementDateUtil.GetMonthShort(from.get(Calendar.MONTH) + 1)
                                + " " + from.get(Calendar.DAY_OF_MONTH) + StatementDateUtil.GetWhetherFuck() +
                                from.get(Calendar.YEAR) + " " +
                                mContext.getResources().getString(R.string.to) + " " +
                                StatementDateUtil.GetMonthShort(to.get(Calendar.MONTH) + 1)
                                + " " + to.get(Calendar.DAY_OF_MONTH)  + StatementDateUtil.GetWhetherFuck() +
...
//        refWatcher.watch(this);
    ...
    private void select() {
        ...
        for (int i = start; i >= end; i--) {
            StatementDateRecord coCoinRecord = RecordManager.RECORDS.get(i);
        ...
        expense.setText(StatementDateUtil.GetInMoney(Sum));
        emptyTip.setVisibility(View.GONE);
        ...
        for (Map.Entry<Integer, Double> entry : TagExpanse.entrySet()) {
            if (entry.getValue() >= 1) {
                SliceValue sliceValue = new SliceValue(
                        ...
                        StatementDateUtil.GetTagColor(entry.getKey()));
...
// set value touch listener of pie//////////////////////////////////////////////////////////////////
        ...
        all.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                List<StatementDateRecord> data = new LinkedList<StatementDateRecord>();
                for (int i = start; i >= end; i--) data.add(RecordManager.RECORDS.get(i));
                if ("zh".equals(StatementDateUtil.GetLanguage())) {
                    dialogTitle = dateShownString + "\n" +
                            StatementDateUtil.GetSpendString(Sum) +
                            "于" + StatementDateUtil.GetTagName(tagId);
                } else {
                    dialogTitle = StatementDateUtil.GetSpendString(Sum) + " "
                            + mContext.getResources().getString(R.string.from) + " " +
                            StatementDateUtil.GetMonthShort(from.get(Calendar.MONTH) + 1) + " " +
                            from.get(Calendar.DAY_OF_MONTH) + " " +
                            from.get(Calendar.YEAR) + "\n" +
