...
public class EditRecordFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        if (fragmentPosition == 0) {
            ...
            editView.setText("" + (int) RecordManager.RECORDS.get(StatementDateUtil.editRecordPosition).getMoney());
            editView.requestFocus();
            editView.setHelperText(StatementDateUtil.FLOATINGLABELS[editView.getText().toString().length()]);

            tagId = RecordManager.RECORDS.get(StatementDateUtil.editRecordPosition).getTag();
            tagName.setText(StatementDateUtil.GetTagName(tagId));
            tagImage.setImageResource(StatementDateUtil.GetTagIcon(tagId));

            remarkEditView.setVisibility(View.GONE);
        } else {
            remarkEditView.setTypeface(StatementDateUtil.GetTypeface());

            remarkEditView.setText(RecordManager.RECORDS.get(StatementDateUtil.editRecordPosition).getRemark());
    ...
    public void setEditColor(boolean shouldChange) {
        if (shouldChange) {
            ...
            editView.setTextColor(StatementDateUtil.MY_BLUE);
            editView.setPrimaryColor(StatementDateUtil.MY_BLUE);
            editView.setHelperTextColor(StatementDateUtil.MY_BLUE);
            remarkEditView.setTextColor(StatementDateUtil.MY_BLUE);
            remarkEditView.setPrimaryColor(StatementDateUtil.MY_BLUE);
            remarkEditView.setHelperTextColor(StatementDateUtil.MY_BLUE);
        }
    }
