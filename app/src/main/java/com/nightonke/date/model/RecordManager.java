...
import com.nightonke.date.db.DB;
...
public class RecordManager {
    ...
    public static List<StatementDateRecord> SELECTED_RECORDS;

    public static Integer SUM;
    public static List<StatementDateRecord> RECORDS;
...
// saveRecord///////////////////////////////////////////////////////////////////////////////////////
    public static long saveRecord(final StatementDateRecord coCoinRecord) {
        long insertId = -1;
...
//        else coCoinRecord.setUserId(null);
        ...
        if (insertId == -1) {
            ...
            StatementDateToast.getInstance()
...
// delete a coCoinRecord//////////////////////////////////////////////////////////////////////////////////
    public static long deleteRecord(final StatementDateRecord coCoinRecord, boolean deleteInList) {
...
//            }
    ...
    public static int deleteTag(int id) {
        ...
        for (StatementDateRecord coCoinRecord : RECORDS) {
    ...
    public static long updateRecord(final StatementDateRecord coCoinRecord) {
        ...
        if (updateNumber <= 0) {
            ...
            StatementDateToast.getInstance().showToast(R.string.update_failed_locale, SuperToast.Background.RED);
        } else {
            if (BuildConfig.DEBUG) {
...
//            }
            db.updateRecord(coCoinRecord);
            StatementDateToast.getInstance().showToast(R.string.update_successfully_locale, SuperToast.Background.BLUE);
        }
        return updateNumber;
    }
...
// already login////////////////////////////////////////////////////////////////////////////////////
            ...
            for (int i = 0; i < RECORDS.size(); i++) {
                ...
                final StatementDateRecord coCoinRecord = RECORDS.get(i);
...
//get records from server to local//////////////////////////////////////////////////////////////////
    ...
    public static long getRecordsFromServer() {
        updateNum = 0;
        BmobQuery<StatementDateRecord> query = new BmobQuery<StatementDateRecord>();
        query.addWhereEqualTo("userId",
                BmobUser.getCurrentUser(CoCoinApplication.getAppContext(), User.class).getObjectId());
        query.setLimit(Integer.MAX_VALUE);
        query.findObjects(CoCoinApplication.getAppContext(), new FindListener<StatementDateRecord>() {
            @Override
            public void onSuccess(List<StatementDateRecord> object) {
                if (BuildConfig.DEBUG) {
                ...
                for (StatementDateRecord coCoinRecord : object) {
                    boolean exist = false;
                    ...
                    if (!exist) {
                        StatementDateRecord newCoCoinRecord = new StatementDateRecord();
                ...
                Collections.sort(RECORDS, new Comparator<StatementDateRecord>() {
                    @Override
    ...
    public static int getCurrentMonthExpense() {
        ...
        Calendar left = StatementDateUtil.GetThisMonthLeftRange(calendar);
        int monthSum = 0;
        for (int i = RECORDS.size() - 1; i >= 0; i--) {
    ...
    public static List<StatementDateRecord> queryRecordByTime(Calendar c1, Calendar c2) {
        List<StatementDateRecord> list = new LinkedList<>();
        for (StatementDateRecord coCoinRecord : RECORDS) {
    ...
    public static List<StatementDateRecord> queryRecordByCurrency(String currency) {
        List<StatementDateRecord> list = new LinkedList<>();
        for (StatementDateRecord coCoinRecord : RECORDS) {
    ...
    public static List<StatementDateRecord> queryRecordByTag(int tag) {
        List<StatementDateRecord> list = new LinkedList<>();
        for (StatementDateRecord coCoinRecord : RECORDS) {
    ...
    public static List<StatementDateRecord> queryRecordByMoney(double money1, double money2, String currency) {
        List<StatementDateRecord> list = new LinkedList<>();
        for (StatementDateRecord coCoinRecord : RECORDS) {
    ...
    public static List<StatementDateRecord> queryRecordByRemark(String remark) {
        List<StatementDateRecord> list = new LinkedList<>();
        for (StatementDateRecord coCoinRecord : RECORDS) {
            if (StatementDateUtil.IsStringRelation(coCoinRecord.getRemark(), remark)) {
                list.add(coCoinRecord);
            }
        }
        return list;
    ...
    private void randomDataCreater() {
        ...
        List<StatementDateRecord> createdCoCoinRecords = new ArrayList<>();
        ...
        while (c.before(now)) {
            for (int i = 0; i < RANDOM_DATA_NUMBER_ON_EACH_DAY; i++) {
                ...
                StatementDateRecord coCoinRecord = new StatementDateRecord();
        ...
        Collections.sort(createdCoCoinRecords, new Comparator<StatementDateRecord>() {
            @Override
            public int compare(StatementDateRecord lhs, StatementDateRecord rhs) {
                if (lhs.getCalendar().before(rhs.getCalendar())) {
                    return -1;
        ...
        for (StatementDateRecord coCoinRecord : createdCoCoinRecords) {
