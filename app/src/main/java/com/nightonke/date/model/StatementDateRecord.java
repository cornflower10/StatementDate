...
public class StatementDateRecord extends BmobObject {
    ...
    public StatementDateRecord() {
        this.id = Long.valueOf(-1);
    }

    public StatementDateRecord(long id, float money, String currency, int tag, Calendar calendar) {
        this.id = id;
        this.money = money;
    ...
    public StatementDateRecord(long id, float money, String currency, int tag, Calendar calendar, String remark) {
        this.id = id;
        this.money = money;
        this.currency = currency;
        this.tag = tag;
    ...
    public void set(StatementDateRecord coCoinRecord) {
    ...
    public boolean isInMoney(double money1, double money2, String currency) {
        return StatementDateUtil.ToDollas(money1, currency) <= StatementDateUtil.ToDollas(this.money, this.currency)
                && StatementDateUtil.ToDollas(money2, currency) > StatementDateUtil.ToDollas(this.money, this.currency);
    }

    public String getCalendarString() {
        return String.format("%02d", calendar.get(Calendar.HOUR_OF_DAY)) + ":"
                + String.format("%02d", calendar.get(Calendar.MINUTE)) + " "
                + StatementDateUtil.GetMonthShort(calendar.get(Calendar.MONTH) + 1) + " "
