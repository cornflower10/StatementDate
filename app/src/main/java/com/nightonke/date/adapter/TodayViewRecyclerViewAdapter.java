...
import com.nightonke.date.fragment.RecordCheckDialogFragment;
...
public class TodayViewRecyclerViewAdapter
    ...
    private ArrayList<StatementDateRecord> allData;
    ...
    public TodayViewRecyclerViewAdapter(int start, int end, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            for (int j = 2; j < size; j++) {
                ...
                Expanse.put(recordManager.TAGS.get(j).getId(), new ArrayList<StatementDateRecord>());
            }

            size = allData.size();
            for (int i = 0; i < size; i++) {
                StatementDateRecord coCoinRecord = allData.get(i);
                ...
                if (axis_date == Calendar.DAY_OF_WEEK) {
                    if (StatementDateUtil.WEEK_START_WITH_SUNDAY)
            ...
            TagExpanse = StatementDateUtil.SortTreeMapByValues(TagExpanse);
        }
    }
    ...
    public void onBindViewHolder(final viewHolder holder, final int position) {
        ...
        switch (getItemViewType(position)) {
            case TYPE_HEADER:
                ...
                holder.expanseSum.setText(StatementDateUtil.GetInMoney((int) Sum));

                holder.date.setTypeface(StatementDateUtil.GetTypeface());
                holder.dateBottom.setTypeface(StatementDateUtil.GetTypeface());
                holder.expanseSum.setTypeface(StatementDateUtil.typefaceLatoLight);

                if (IS_EMPTY) {
                    holder.emptyTip.setVisibility(View.VISIBLE);
                    holder.emptyTip.setText(StatementDateUtil.GetTodayViewEmptyTip(fragmentPosition));
                    holder.emptyTip.setTypeface(StatementDateUtil.GetTypeface());
                        ...
                        if (entry.getValue() >= 1) {
                            SliceValue sliceValue = new SliceValue(
                                    ...
                                    mContext.getApplicationContext().getResources().
                                            getColor(StatementDateUtil.GetTagColorResource(entry.getKey())));
                    ...
                    if (!(fragmentPosition == TODAY || fragmentPosition == YESTERDAY)) {
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            ...
                            SubcolumnValue value = new SubcolumnValue(
                                    originalTargets[i], StatementDateUtil.GetRandomColor());
                        ...
                        for (int i = 0; i < columnNumber; i++) {
                            axisValueList.add(
                                    new AxisValue(i).setLabel(StatementDateUtil.GetAxisDateName(axis_date, i)));
                        }
                        ...
                        public void onValueSelected(int p, SliceValue sliceValue) {
                            ...
                            Snackbar snackbar =
                                    Snackbar
                                            ...
                                            .margin(15, 15)
                                            .backgroundDrawable(StatementDateUtil.GetSnackBarBackground(
                                                    fragmentPosition - 2))
                                            .text(text)
...
// histogram data///////////////////////////////////////////////////////////////////////////////////
                                ...
                                for (int i = Expanse.get(tagId).size() - 1; i >= 0; i--) {
                                    StatementDateRecord coCoinRecord = Expanse.get(tagId).get(i);
                                    if (axis_date == Calendar.DAY_OF_WEEK) {
                                        if (StatementDateUtil.WEEK_START_WITH_SUNDAY) {
...
// set value touch listener of histogram////////////////////////////////////////////////////////////
                        holder.histogram.setOnValueTouchListener(
                            ...
                            public void onValueSelected(int columnIndex,
                                ...
                                String text = StatementDateUtil.GetSpendString((int) value.getValue());
                                if (tagId != -1)
                                    // belongs a tag
                                    if ("zh".equals(StatementDateUtil.GetLanguage()))
...
// set the listener of the show all button//////////////////////////////////////////////////////////
            ...
            case TYPE_BODY:
                ...
                holder.tagImage.setImageResource(
                        StatementDateUtil.GetTagIcon(allData.get(position - 1).getTag()));
                ...
                holder.cell_date.setTypeface(StatementDateUtil.typefaceLatoLight);
                ...
                holder.layout.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        dialog = new MaterialDialog.Builder(mContext)
                                .icon(StatementDateUtil.GetTagIconDrawable(allData.get(position - 1).getTag()))
...
// set the listener of the check button on the snack bar of pie/////////////////////////////////////
    private class mActionClickListenerForPie implements ActionClickListener {
        ...
        public void onActionClicked(Snackbar snackbar) {
            List<StatementDateRecord> shownCoCoinRecords = Expanse.get(tagId);
            ((FragmentActivity)mContext).getSupportFragmentManager()
                    .beginTransaction()
                    .add(new RecordCheckDialogFragment(
...
// set the listener of the check button on the snack bar of histogram///////////////////////////////
    private class mActionClickListenerForHistogram implements ActionClickListener {
        @Override
        public void onActionClicked(Snackbar snackbar) {
            ArrayList<StatementDateRecord> shownCoCoinRecords = new ArrayList<>();
            int index = timeIndex;
            if (axis_date == Calendar.DAY_OF_WEEK) {
                if (StatementDateUtil.WEEK_START_WITH_SUNDAY) index++;
                else
                    if (index == 6) index = 1;
                    else index += 2;
            }
        ...
        switch (fragmentPosition) {
            ...
            case THIS_WEEK:
                ...
                return mContext.getResources().getString(R.string.on)
                        + StatementDateUtil.GetWeekDay(timeIndex);
            case LAST_WEEK:
                // 在上周一
                // on last Monday
                return mContext.getResources().getString(R.string.on)
            ...
            case THIS_MONTH:
                ...
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
                        StatementDateUtil.GetMonthShort(month) + StatementDateUtil.GetWhetherBlank() +
                        (timeIndex + 1) + StatementDateUtil.GetWhetherFuck();
            case LAST_MONTH:
                // 在1月1日
                // on Jan. 1
                return mContext.getResources().getString(R.string.on) +
            ...
            case THIS_YEAR:
                if ("zh".equals(StatementDateUtil.GetLanguage()))
                    ...
                    return mContext.getResources().getString(R.string.in) +
                            ...
                            StatementDateUtil.GetMonthShort(timeIndex + 1);
                else
                    // in Jan. 1
                    return mContext.getResources().getString(R.string.in) +
                            StatementDateUtil.GetMonthShort(timeIndex + 1) + " " +
...
// set the dateString of this fragment//////////////////////////////////////////////////////////////
    private void setDateString() {
        ...
        Calendar yesterday = StatementDateUtil.GetYesterdayLeftRange(today);
        basicTodayDateString = "--:-- ";
        basicTodayDateString += StatementDateUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                + " " + today.get(Calendar.DAY_OF_MONTH) + " " +
                today.get(Calendar.YEAR);
        basicYesterdayDateString = "--:-- ";
        basicYesterdayDateString += StatementDateUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
        ...
        switch (fragmentPosition) {
            ...
            case YESTERDAY:
                ...
                break;
            case THIS_WEEK:
                Calendar leftWeekRange = StatementDateUtil.GetThisWeekLeftRange(today);
                Calendar rightWeekRange = StatementDateUtil.GetThisWeekRightShownRange(today);
                dateString = StatementDateUtil.GetMonthShort(leftWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftWeekRange.get(Calendar.YEAR) + " - " +
                        StatementDateUtil.GetMonthShort(rightWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case LAST_WEEK:
                Calendar leftLastWeekRange = StatementDateUtil.GetLastWeekLeftRange(today);
                Calendar rightLastWeekRange = StatementDateUtil.GetLastWeekRightShownRange(today);
                dateString
                        = StatementDateUtil.GetMonthShort(leftLastWeekRange.get(Calendar.MONTH) + 1)
                        + " " + leftLastWeekRange.get(Calendar.DAY_OF_MONTH) + " " +
                        leftLastWeekRange.get(Calendar.YEAR) + " - " +
                        StatementDateUtil.GetMonthShort(rightLastWeekRange.get(Calendar.MONTH) + 1)
                ...
                break;
            case THIS_MONTH:
                dateString = StatementDateUtil.GetMonthShort(today.get(Calendar.MONTH) + 1)
                        + " " + today.get(Calendar.YEAR);
                dateShownString
                ...
                break;
            case LAST_MONTH:
                Calendar lastMonthCalendar = StatementDateUtil.GetLastMonthLeftRange(today);
                dateString
                        = StatementDateUtil.GetMonthShort(lastMonthCalendar.get(Calendar.MONTH) + 1)
            ...
            case THIS_YEAR:
                ...
                break;
            case LAST_YEAR:
                Calendar lastYearCalendar = StatementDateUtil.GetLastYearLeftRange(today);
    ...
    private String getAllDataDialogTitle() {
        ...
        if ("zh".equals(StatementDateUtil.GetLanguage())) {
            ...
            postfix = StatementDateUtil.GetSpendString((int)Sum);
        } else {
            prefix = StatementDateUtil.GetSpendString((int)Sum);
            postfix = "";
        }
        switch (fragmentPosition) {
            case TODAY:
