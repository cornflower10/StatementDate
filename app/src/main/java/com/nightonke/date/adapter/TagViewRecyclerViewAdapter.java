...
import com.nightonke.date.model.Tag;
...
public class TagViewRecyclerViewAdapter
    ...
    private List<List<StatementDateRecord>> contents;
    ...
    public TagViewRecyclerViewAdapter(List<StatementDateRecord> coCoinRecords, Context context, int position) {
        ...
        if (!IS_EMPTY) {
            ...
            Collections.sort(coCoinRecords, new Comparator<StatementDateRecord>() {
                @Override
                public int compare(StatementDateRecord lhs, StatementDateRecord rhs) {
            ...
            double monthSum = 0;
            double yearSum = 0;
            List<StatementDateRecord> yearSet = new ArrayList<>();
            List<StatementDateRecord> monthSet = new ArrayList<>();

            for (StatementDateRecord coCoinRecord : coCoinRecords) {
            ...
            if (chartType == PIE) {
                ...
                for (int i = 0; i < contents.size(); i++) {
                    ...
                    for (StatementDateRecord coCoinRecord : contents.get(i)) {
                ...
                if (IS_EMPTY) {
                    ...
                    holder.from.setText(
                            mContext.getResources().getString(R.string.from) + " " +
                                    startYear + " " + StatementDateUtil.GetMonthShort(startMonth));
                    holder.sum.setText(StatementDateUtil.GetInMoney((int)Sum));
                    holder.to.setText(
                            mContext.getResources().getString(R.string.to) + " " +
                                    endYear + " " + StatementDateUtil.GetMonthShort(endMonth));
                    holder.to.setTypeface(StatementDateUtil.GetTypeface());
                    holder.from.setTypeface(StatementDateUtil.GetTypeface());
                }
                holder.sum.setTypeface(StatementDateUtil.typefaceLatoLight);
                break;
            case TYPE_CELL:
                ...
                holder.expanse.setTypeface(StatementDateUtil.GetTypeface());
                switch (chartType) {
                    case PIE:
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_MONTH)) {
                            holder.date.setText(year + " " + StatementDateUtil.GetMonthShort(month));
                        } else {
                            holder.date.setText(year + " ");
                        }

                        holder.expanse.setText(StatementDateUtil.GetInMoney((int) (double) SumList.get(position - 1)));
                        ...
                        if (type.get(position - 1).equals(SHOW_IN_YEAR)) {
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                ...
                                SubcolumnValue value = new SubcolumnValue(
                                        ...
                                        StatementDateUtil.GetRandomColor());
                                value.setLabel(StatementDateUtil.MONTHS_SHORT[month] + " " + year);
                            ...
                            for (int i = 0; i < numColumns; i++) {
                                axisValueList.add(new AxisValue(i)
                                        .setLabel(StatementDateUtil.GetMonthShort(i + 1)));
                            }
                            ...
                            for (int i = 0; i < numColumns; ++i) {
                                ...
                                SubcolumnValue value = new SubcolumnValue((float)
                                        DayExpanseSum[(year - startYear) * 372
                                                + (month - 1) * 31 + i],
                                        StatementDateUtil.GetRandomColor());
                                value.setLabel(StatementDateUtil.MONTHS_SHORT[month] + " " + (i + 1) + " " + year);
                                subcolumnValues.add(value);
                                Column column = new Column(subcolumnValues);
                                column.setHasLabels(false);
                                column.setHasLabelsOnlyForSelected(false);
        ...
        public void onValueSelected(final int columnIndex, int subColumnIndex, SubcolumnValue value) {
            Snackbar snackbar =
                    Snackbar.with(mContext)
                            ...
                            .margin(15, 15)
                            .backgroundDrawable(StatementDateUtil.GetSnackBarBackground(fragmentTagId))
                            .textColor(Color.WHITE)
                            .textTypeface(StatementDateUtil.GetTypeface())
                            .actionLabel(mContext.getResources().getString(R.string.check))
                            .actionLabelTypeface(StatementDateUtil.typefaceLatoLight)
            ...
            if (fragmentPosition == SUM_HISTOGRAM) {
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    timeString = " " + StatementDateUtil.GetMonthShort(month)
                            + " " + (columnIndex + 1) + " "
                    ...
                    if ("zh".equals(StatementDateUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString + "\n" +
                                StatementDateUtil.GetSpendString((int) value.getValue());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                StatementDateUtil.GetSpendString((int) value.getValue());
                    } else {
                        text = StatementDateUtil.GetSpendString((int) value.getValue()) + "\n" +
                                mContext.getResources().getString(R.string.on) + timeString;
                        dialogTitle = StatementDateUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString;
                    }
                    snackbar.text(text);
                    snackbar.actionListener(new ActionClickListener() {
                        ...
                        public void onActionClicked(Snackbar snackbar) {
                            List<StatementDateRecord> shownCoCoinRecords = new ArrayList<>();
                            boolean isSamed = false;
                            for (StatementDateRecord coCoinRecord : contents.get(position)) {
                ...
                if (type.get(position).equals(SHOW_IN_YEAR)) {
                    ...
                    timeString = " " + StatementDateUtil.GetMonthShort(columnIndex + 1) + " "
                            + timeString.substring(timeString.length() - 4, timeString.length());
                    if ("zh".equals(StatementDateUtil.GetLanguage())) {
                ...
                if (type.get(position).equals(SHOW_IN_MONTH)) {
                    ...
                    if ("zh".equals(StatementDateUtil.GetLanguage())) {
                        text = mContext.getResources().getString(R.string.on) + timeString +
                                StatementDateUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + StatementDateUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = mContext.getResources().getString(R.string.on) + timeString +
                                StatementDateUtil.GetSpendString((int) value.getValue()) + "\n" +
                                "于" + StatementDateUtil.GetTagName(contents.get(position).get(0).getTag());
                    } else {
                        text = StatementDateUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + StatementDateUtil.GetTagName(contents.get(position).get(0).getTag());
                        dialogTitle = StatementDateUtil.GetSpendString((int) value.getValue()) +
                                mContext.getResources().getString(R.string.on) + timeString + "\n"
                                + "in " + StatementDateUtil.GetTagName(contents.get(position).get(0).getTag());
                    }
    ...
    private class PieValueTouchListener implements PieChartOnValueSelectListener {
        ...
        public void onValueSelected(int i, SliceValue sliceValue) {
            ...
            if ("zh".equals(StatementDateUtil.GetLanguage())) {
                ...
                text = StatementDateUtil.GetSpendString((int) sliceValue.getValue()) +
                        StatementDateUtil.GetPercentString(percent) + "\n" +
                        "in " + StatementDateUtil.GetTagName(RecordManager.TAGS.get(tagId).getId());
                dialogTitle = StatementDateUtil.GetSpendString((int) sliceValue.getValue()) +
                        mContext.getResources().getString(R.string.in) + timeString + "\n" +
                        "in " + StatementDateUtil.GetTagName(RecordManager.TAGS.get(tagId).getId());
            }
            Snackbar snackbar =
                    Snackbar
                            ...
                            .actionListener(new ActionClickListener() {
                                ...
                                public void onActionClicked(Snackbar snackbar) {
                                    List<StatementDateRecord> shownCoCoinRecords = new ArrayList<>();
                                    for (StatementDateRecord record : contents.get(position)) {
