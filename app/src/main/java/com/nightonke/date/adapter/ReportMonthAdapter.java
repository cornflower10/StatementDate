...
public class ReportMonthAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        name.setText(StatementDateUtil.GetMonthShort((int) highestMonthExpense.get(position + 1)[1] + 1) + " " + year + StatementDateUtil.getInstance().GetPurePercentString(highestMonthExpense.get(position + 1)[4] * 100));
        expense.setText(StatementDateUtil.getInstance().GetInMoney((int) highestMonthExpense.get(position + 1)[3]));
        records.setText(StatementDateUtil.getInstance().GetInRecords((int) highestMonthExpense.get(position + 1)[5]));
