...
public class ReportDayAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        icon.setTypeface(StatementDateUtil.getInstance().typefaceLatoLight);
        name.setTypeface(StatementDateUtil.getInstance().typefaceLatoLight);
        expense.setTypeface(StatementDateUtil.getInstance().typefaceLatoLight);
        records.setTypeface(StatementDateUtil.getInstance().typefaceLatoLight);

        icon.setBackgroundResource(getBackgroundResource());
        icon.setText("" + ((int)dayExpense.get(position + 1)[2]));
        name.setText(StatementDateUtil.getInstance().GetCalendarStringDayExpenseSort(convertView.getContext(), (int)dayExpense.get(position + 1)[0], (int)dayExpense.get(position + 1)[1] + 1, (int)dayExpense.get(position + 1)[2]) + StatementDateUtil.getInstance().GetPurePercentString(dayExpense.get(position + 1)[4] * 100));
        expense.setText(StatementDateUtil.getInstance().GetInMoney((int) dayExpense.get(position + 1)[3]));
        records.setText(StatementDateUtil.getInstance().GetInRecords((int) dayExpense.get(position + 1)[5]));
